-- 018: Fire card theme
--
-- A fifth 'set the tone' option: the Fire theme, which sets the owner's
-- numbered cards ablaze with a flickering flame background, a burning
-- number, rising embers, and smoke drifting off the top. Recreates the
-- purchase/select RPCs from 017 with 'fire' added to the allowlist so the
-- id is recognized server-side. Safe to run whether or not 017 has been
-- applied yet (create or replace).
...
begin
  ...
  if p_font_id not in ('pixel','fancy','display','serif','fire') then
