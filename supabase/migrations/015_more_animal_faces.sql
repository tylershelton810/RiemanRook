-- 015: More animal faces
--
-- Extends the crow-face shop with animal silhouettes (fox, owl, cat, panda)
-- at the same 10-token price as the other paid faces. Recreates the
-- purchase/select RPCs from 013 so the server recognizes the new ids; safe
-- to run whether or not 013 has been applied yet.
...
begin
  ...
  if p_logo_id not in ('bird','party','cool','crown','chef','fox','owl','cat','panda') then
...
begin
  ...
  if p_logo_id in ('bird','party','cool','crown','chef','fox','owl','cat','panda') then
