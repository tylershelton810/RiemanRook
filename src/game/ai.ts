...
import { riemanRules, riemanRules5Handed } from '../rules/riemanRules'
...
const CALL_TEN_MIN_HIGH = 3
const OPENING_BAR_5 = 26
const CEILING_FLOOR_STRENGTH_5 = 18
...
export function chooseAiCardWithKnowledge(hand: Card[], knowledge: HandKnowledge, aiPlayer?: PlayerState, bidder?: PlayerState, players: PlayerState[] = []) {
  ...
  if (currentWinnerIsPartner) {
    const isBidderTeam = Boolean(aiPlayer && bidder && aiPlayer.team === bidder.team)
    if (isBidderTeam && winningCard.card.kind !== 'crow') {
      const crow = legalCards.find(card => card.kind === 'crow')
      if (crow) {
        const winningIsTrump = winningCard.card.kind === 'number' && winningCard.card.color === trumpColor
        const winningIsHigh = winningCard.card.kind === 'number' && winningCard.card.value >= 13
        if (!winningIsTrump || !winningIsHigh) return crow
      }
    }
  ...
  if (winningCards.length) {
    ...
    const crowInPlay = !knowledge.trumpCardsPlayed.some(card => card.kind === 'crow')
    if (crowInPlay && winningTrump.length && !winningTrump.some(card => card.kind === 'crow')) {
      const safeCards = legalCards.filter(card => !winningTrump.includes(card))
      if (safeCards.length) return lowestCard(safeCards)
    }
...
function highestRemainingTrump(knowledge: HandKnowledge, trumpColor: CardColor): number {
  const crowPlayed = knowledge.trumpCardsPlayed.some(card => card.kind === 'crow')
  if (!crowPlayed) return 15
...
export function chooseAiBid5(hand: Card[], currentBid: number | null, aiPlayer: PlayerState) {
  const best = COLORS.reduce((winner, color) => {
    const candidate = evaluateTrumpSuit(hand, color)
    return candidate.score > winner.score ? candidate : winner
  }, evaluateTrumpSuit(hand, COLORS[0]))
  ...
  const dumpNudge = best.high >= DUMP_NUDGE_TRUMP_HIGH && best.dump >= DUMP_NUDGE_MIN_DUMP ? 5 : 0
  const crowDepthNudge = best.crow && best.depth >= CROW_DEPTH_NUDGE_MIN_DEPTH ? 5 : 0
  const floor = riemanRules5Handed.bidding.minimum
  const ceiling = Math.min(riemanRules5Handed.bidding.maximum, Math.max(floor, floor + Math.floor(Math.max(0, best.control - CEILING_FLOOR_STRENGTH_5) / 5) * 5 + dumpNudge + crowDepthNudge + riskShift))
  if (best.score < OPENING_BAR_5) return null
  if (currentBid === null) return floor
  const nextBid = currentBid + riemanRules5Handed.bidding.increment
  return nextBid <= ceiling ? nextBid : null
}

export function chooseAiPartnerCard5(hand: Card[]): Card {
  const held = new Set(hand.map((card) => card.id))
  if (!held.has('crow')) return { id: 'crow', kind: 'crow' }
  const candidates = COLORS.map((color) => {
    const values: number[] = []
    for (let value = 14; value >= 1; value -= 1) {
      const id = `${color}-${value}`
      if (!held.has(id)) values.push(value)
    }
    const suitHand = hand.filter((card): card is NumberCard => card.kind === 'number' && card.color === color)
    const highCount = suitHand.filter((card) => HIGH_CARD_STRENGTH[card.value] > 0).length
    return { color, values, score: evaluateTrumpSuit(hand, color).score, highCount }
  }).filter((entry) => entry.values.length > 0).sort((a, b) => b.score - a.score)
  const pick = candidates[0]
  if (pick) {
    if (pick.highCount >= CALL_TEN_MIN_HIGH && pick.values.includes(10)) return { id: `${pick.color}-10`, kind: 'number', color: pick.color, value: 10 }
    return { id: `${pick.color}-${pick.values[0]}`, kind: 'number', color: pick.color, value: pick.values[0] }
  }
  return { id: 'crow', kind: 'crow' }
}

export function chooseAiDiscard5(hand: Card[]) {
  return planKitty(hand).discardIds.slice(0, riemanRules5Handed.deal.kittySize)
}

export function buildHandKnowledge5(trick: Trick | undefined, trumpColor: CardColor | undefined, bidder: PlayerState | undefined, completedTricks: Trick[], five: { calledCard?: Card; partnerId?: string; partnershipRevealed?: boolean; alone?: boolean } = {}): HandKnowledge {
  return { ...buildHandKnowledge(trick, trumpColor, bidder, completedTricks), ...five }
}

export function chooseAiCard5(hand: Card[], knowledge: HandKnowledge, aiPlayer?: PlayerState, players: PlayerState[] = []) {
  const trick = knowledge.currentTrick
  const trumpColor = knowledge.trumpColor
  const leadColor = leadColorForTrick(trick, trumpColor)
  const legalCards = hand.filter((card) => canPlayCard(hand, card, leadColor, trumpColor))
  const pick = !trick || trick.cards.length === 0
    ? chooseLeadCard5(hand, knowledge, aiPlayer, players)
    : chooseFollowCard5(legalCards, knowledge, aiPlayer, players)
  return applyCallConstraint(pick, hand, knowledge, aiPlayer)
}

function chooseFollowCard5(legalCards: Card[], knowledge: HandKnowledge, aiPlayer?: PlayerState, players: PlayerState[] = []) {
  const trick = knowledge.currentTrick
  const trumpColor = knowledge.trumpColor
  const leadColor = leadColorForTrick(trick, trumpColor)
  const winningCard = trick.cards.reduce((winner, played) => cardBeats(played.card, winner.card, leadColor, trumpColor) ? played : winner, trick.cards[0])
  const winningCards = legalCards.filter((card) => cardBeats(card, winningCard.card, leadColor, trumpColor))
  const currentWinnerPlayer = players.find((player) => player.id === knowledge.currentWinnerId)
  const currentWinnerIsPartner = isKnownTeammate(currentWinnerPlayer, aiPlayer, knowledge)
  if (currentWinnerIsPartner) {
    const isBidderTeam = Boolean(aiPlayer && (aiPlayer.id === knowledge.bidderId || aiPlayer.id === knowledge.partnerId))
    if (isBidderTeam && winningCard.card.kind !== 'crow') {
      const crow = legalCards.find(card => card.kind === 'crow')
      if (crow) {
        const winningIsTrump = winningCard.card.kind === 'number' && winningCard.card.color === trumpColor
...
function chooseLeadCard5(hand: Card[], knowledge: HandKnowledge, aiPlayer?: PlayerState, players: PlayerState[] = []): Card {
  const trumpColor = knowledge.trumpColor
  const amBidder = Boolean(aiPlayer && aiPlayer.id === knowledge.bidderId)
  const madeTrump = amBidder || (aiPlayer?.id === knowledge.partnerId && knowledge.partnershipRevealed)
  if (madeTrump && trumpColor) {
    const myTrump = hand.filter((card) => card.kind === 'crow' || (card.kind === 'number' && card.color === trumpColor))
    const trumpLeft = TRUMP_TOTAL - myTrump.length - knowledge.trumpCardsPlayed.length
    const opponentsOut = trumpLeft <= 0 || opponentsVoidInTrump5(knowledge, aiPlayer, players)
    if (!opponentsOut && myTrump.length) {
      const myHighest = highestCard(myTrump)
      const myHighestValue = myHighest.kind === 'crow' ? 15 : myHighest.value
      const currentHighest = highestRemainingTrump(knowledge, trumpColor)
...
function opponentsVoidInTrump5(knowledge: HandKnowledge, aiPlayer: PlayerState | undefined, players: PlayerState[]): boolean {
  const trumpColor = knowledge.trumpColor
  if (!trumpColor || !aiPlayer) return false
  const opponents = players.filter((player) => !isKnownTeammate(player, aiPlayer, knowledge))
  return opponents.length > 0 && opponents.every((opponent) => (knowledge.voidColorsByPlayer[opponent.id] ?? []).includes(trumpColor))
}

function isKnownTeammate(player: PlayerState | undefined, aiPlayer: PlayerState | undefined, knowledge: HandKnowledge): boolean {
  if (!player || !aiPlayer || player.id === aiPlayer.id) return false
  const amPartner = aiPlayer.id === knowledge.partnerId
  const amBidder = aiPlayer.id === knowledge.bidderId
  if (amPartner) return player.id === knowledge.bidderId
  if (amBidder) return Boolean(knowledge.partnershipRevealed) && player.id === knowledge.partnerId
  return false
}

function applyCallConstraint(pick: Card, hand: Card[], knowledge: HandKnowledge, aiPlayer?: PlayerState): Card {
  if (!aiPlayer || aiPlayer.id !== knowledge.partnerId || knowledge.partnershipRevealed) return pick
  const calledId = knowledge.calledCard?.id
  const isTrump = pick.kind === 'crow' || (pick.kind === 'number' && pick.color === knowledge.trumpColor)
  if (!isTrump) return pick
  if (pick.id === calledId) return pick
  const called = hand.find((card) => card.id === calledId)
  return called ?? pick
}
