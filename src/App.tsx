...
import { bidOn, callOn, closeLobby, dealNextHand, discardOn, getActiveGameSession, getCurrentGameSession, getLeaderboard, getPlayerStatistics, getProfileNames, isFiveHanded, nextHandOn, persistLocalGame, playCardOn, reconcileAiSeats, rematchOn, rematchSession, startGameSession, submitBid, submitCall, submitTrump, submitDiscard, submitCard, trumpOn } from './services/sessions'
import type { LeaderboardEntry, LeaderboardNumbers } from './services/sessions'
import { createConfetti } from './game/celebration'
import type { Card, PlayerState, RulesetId, SessionState, Team } from './game/types'
import type { CardColor } from './game/types'
import { buildHandKnowledge, buildHandKnowledge5, chooseAiBid, chooseAiBid5, chooseAiCard5, chooseAiCardWithKnowledge, chooseAiDiscard5, chooseAiPartnerCard5, chooseAiTrump, planKitty } from './game/ai'
import { canPlayCard, leadColorForTrick, pointsInCards } from './game/rules'
import { capturedPointsForTeam, createSession } from './game/session'
import { createSession5 } from './game/session5'

const avatarColors = ['coral', 'gold', 'sage', 'lavender', 'sky']

function seatTeamForIndex(index: number, seatCount: number): LobbySeat['team'] {
  if (seatCount === 5) return 'ABCDE'[index] as LobbySeat['team']
  return index % 2 === 0 ? 'A' : 'B'
}

function buildSeatModel(ruleset: RulesetId): LobbySeat[] {
  const seatCount = ruleset === 'rieman-rules-5' ? 5 : 4
  return Array.from({ length: seatCount }, (_, index) => (index === 0
    ? { id: 'you', name: 'You', status: 'human' as const, team: 'A' as const }
    : { id: `seat-${index}`, name: 'Open seat', status: 'open' as const, team: seatTeamForIndex(index, seatCount) }))
}
...
function teamLabel(team: Team, game: SessionState, currentUserId?: string) {
...
type PlayerStatistics = {
  ...
  games5_won: number
  games5_lost: number
  games5_unfinished: number
  games5_completed: number
  hands5_played: number
  hands5_bid: number
  winning5_bids: number
  favorite5_colors: Record<string, number>
  favorite5_partners: Record<string, number>
  ai_games_won: number
  ...
  ai_games5_won: number
  ai_games5_lost: number
  ai_games5_unfinished: number
  ai_games5_completed: number
  ai_hands5_played: number
  ai_hands5_bid: number
  ai_winning5_bids: number
  ai_favorite5_colors: Record<string, number>
  ai_favorite5_partners: Record<string, number>
}
...
function App() {
  ...
  const [rulesetId, setRulesetId] = useState<RulesetId>('rieman-rules')
  const [localGame, setLocalGame] = useState(false)
  const [seats, setSeats] = useState<LobbySeat[]>(buildSeatModel('rieman-rules'))
  ...
  useEffect(() => {
    if (!session?.user) return
    getPlayerStatistics(session.user.id).then((stats) => {
      setPlayerStats(stats)
      const partnerIds = new Set<string>([
        ...Object.keys(stats?.favorite_partners ?? {}),
        ...Object.keys(stats?.ai_favorite_partners ?? {}),
        ...Object.keys(stats?.favorite5_partners ?? {}),
        ...Object.keys(stats?.ai_favorite5_partners ?? {}),
      ])
      if (partnerIds.size) getProfileNames([...partnerIds]).then(setPartnerNames).catch(() => undefined)
    }).catch(() => undefined)
  }, [session, view])
  ...
  useEffect(() => {
    ...
    const refreshMembers = async () => {
      ...
      try {
        const snapshot = await getLobbySnapshot(activeLobbyId)
        if (seq !== refreshSeqRef.current) return
        setSeats(snapshot)
        getLobbyMemberLogos(activeLobbyId).then((logos) => { if (seq === refreshSeqRef.current) setCrowLogosByPlayer(logos) }).catch(() => undefined)
        getLobbyMemberPlacements(activeLobbyId).then((placements) => { if (seq === refreshSeqRef.current) setPlacementsByPlayer(placements) }).catch(() => undefined)
        getLobbyMemberFonts(activeLobbyId).then((fonts) => { if (seq === refreshSeqRef.current) setFontsByPlayer(fonts) }).catch(() => undefined)
      ...
      } catch (error) { if (seq === refreshSeqRef.current) showToast(error instanceof Error ? error.message : 'Unable to refresh the lobby.') }
    }
    refreshMembers()
  ...
  useEffect(() => {
    ...
    if (!activeGame || !hand || !activeGameSessionId || !session?.user || session.user.id !== activeLobbyHostId || !['bidding', 'calling', 'trump', 'kitty', 'playing'].includes(hand.phase)) return
    const currentPlayer = activeGame.players[hand.currentPlayerIndex]
    if (!currentPlayer?.isAi) return
    const fiveHanded = isFiveHanded(activeGame)
    const playerCount = activeGame.players.length
    const bidder = activeGame.players.find((player) => player.id === hand.bidderId)
    const trick = hand.tricks[hand.tricks.length - 1]
    const trickComplete = trick?.cards.length === playerCount
    const knowledge = hand.phase === 'playing'
      ? (fiveHanded
        ? buildHandKnowledge5(trickComplete ? undefined : trick, hand.trumpColor, bidder, trickComplete ? hand.tricks : hand.tricks.slice(0, -1), { calledCard: hand.calledCard, partnerId: hand.partnerId, partnershipRevealed: hand.partnershipRevealed, alone: hand.alone })
        : buildHandKnowledge(trickComplete ? undefined : trick, hand.trumpColor, bidder, trickComplete ? hand.tricks : hand.tricks.slice(0, -1)))
      : undefined
    const aiTimer = window.setTimeout(() => {
      const attempt = async () => {
        const submit = async (action: (state: SessionState) => SessionState, remote: (state: SessionState) => Promise<SessionState>) => {
          if (localGame) { setActiveGame(action(structuredClone(activeGame))); return }
          setActiveGame(await remote(activeGame))
        }
        if (hand.phase === 'bidding') {
          const amount = fiveHanded ? chooseAiBid5(currentPlayer.hand, hand.currentBid, currentPlayer) : chooseAiBid(currentPlayer.hand, hand.currentBid, currentPlayer, activeGame.players, hand)
          await submit((state) => bidOn(state, currentPlayer.id, amount), (state) => submitBid(activeGameSessionId, state, currentPlayer.id, amount))
        } else if (hand.phase === 'calling') {
          const card = chooseAiPartnerCard5(currentPlayer.hand)
          await submit((state) => callOn(state, currentPlayer.id, card.id), (state) => submitCall(activeGameSessionId, state, currentPlayer.id, card.id))
          ...
          await submit((state) => trumpOn(state, currentPlayer.id, planned), (state) => submitTrump(activeGameSessionId, state, currentPlayer.id, planned))
        } else if (hand.phase === 'kitty') {
          if (fiveHanded) {
            const discardIds = chooseAiDiscard5(currentPlayer.hand)
            await submit((state) => discardOn(state, currentPlayer.id, discardIds), (state) => submitDiscard(activeGameSessionId, state, currentPlayer.id, discardIds))
          } else {
            const plan = planKitty(currentPlayer.hand)
            plannedTrumpRef.current = plan.trump
            await submit((state) => discardOn(state, currentPlayer.id, plan.discardIds), (state) => submitDiscard(activeGameSessionId, state, currentPlayer.id, plan.discardIds))
          }
        } else if (hand.phase === 'playing') {
          const card = fiveHanded
            ? chooseAiCard5(currentPlayer.hand, knowledge!, currentPlayer, activeGame.players)
            : chooseAiCardWithKnowledge(currentPlayer.hand, knowledge!, currentPlayer, bidder, activeGame.players)
          await submit((state) => playCardOn(state, currentPlayer.id, card.id), (state) => submitCard(activeGameSessionId, state, currentPlayer.id, card.id))
        }
      }
      attempt().catch((error) => showToast(error instanceof Error ? error.message : 'Crow AI could not complete its turn.'))
    }, hand.phase === 'playing' && trickComplete && trick?.visibleUntil ? Math.max(500, trick.visibleUntil - Date.now()) : 500)
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (localGame) { setActiveGame(nextHandOn(structuredClone(activeGame))); return }
  ...
  const switchRuleset = (next: RulesetId) => {
    setRulesetId(next)
    if (!activeLobbyId) setSeats(buildSeatModel(next))
  }
  ...
  const startLobby = async () => {
    ...
    try {
      ...
      const lobby = await createLobby(session.user.id, lobbyName.trim(), rulesetId)
  ...
  const startGame = async () => {
    ...
    setLocalGame(false)
    try {
      const started = await startGameSession(activeLobbyId, session.user.id, seats, timer, winningScore, rulesetId)
      setActiveGameSessionId(started.id)
      setActiveGame(started.game_state)
      setView('game')
  ...
  const handleBid = async (amount: number | null) => {
    ...
    if (localGame) {
      try { setActiveGame(bidOn(structuredClone(activeGame), userId, amount)) } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to submit bid.') }
      return
    }
  ...
  const handleTrump = async (color: CardColor) => {
    ...
    const userId = session.user.id
    if (localGame) {
      try { setActiveGame(trumpOn(structuredClone(activeGame), userId, color)) } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to choose trump.') }
      return
    }
  ...
  const handleDiscard = async (cardIds: string[]) => {
    ...
    const userId = session.user.id
    if (localGame) {
      try { setActiveGame(discardOn(structuredClone(activeGame), userId, cardIds)) } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to discard those cards.') }
      return
    }
  ...
  const handleCard = async (cardId: string) => {
    ...
    const userId = session.user.id
    if (localGame) {
      try { setActiveGame(playCardOn(structuredClone(activeGame), userId, cardId)) } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to play that card.') }
      return
    }
  ...
  const handleCall = async (cardId: string) => {
    if (!activeGameSessionId || !session?.user || !activeGame) return
    const userId = session.user.id
    if (localGame) {
      try { setActiveGame(callOn(structuredClone(activeGame), userId, cardId)) } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to name that card.') }
      return
    }
    try { setActiveGame(await submitCall(activeGameSessionId, activeGame, userId, cardId)) } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to name that card.') }
  }

  const handleRematch = async () => {
    if (!activeLobbyId) return
    if (localGame && activeGame) {
      const rematchState = { ...rematchOn(activeGame), id: crypto.randomUUID() }
  ...
  const totalGamesWon = playerStats ? (playerStats.games_won ?? 0) + (playerStats.ai_games_won ?? 0) + (playerStats.games5_won ?? 0) + (playerStats.ai_games5_won ?? 0) : null
  const totalGamesCompleted = playerStats ? (playerStats.games_completed ?? 0) + (playerStats.ai_games_completed ?? 0) + (playerStats.games5_completed ?? 0) + (playerStats.ai_games5_completed ?? 0) : null
  const totalHandsPlayed = playerStats ? (playerStats.hands_played ?? 0) + (playerStats.ai_hands_played ?? 0) + (playerStats.hands5_played ?? 0) + (playerStats.ai_hands5_played ?? 0) : null
  const leaderboardTop = useMemo(() => {
    if (!leaderboard?.length) return []
    return [...leaderboard]
      .map((entry) => ({ entry, value: entry.stats.gamesWon + entry.five.gamesWon + entry.ai.gamesWon + entry.aiFive.gamesWon }))
  ...
  return <main className="app-shell">
    ...
    {showStats && <StatsModal stats={playerStats} partnerNames={partnerNames} onClose={() => setShowStats(false)} />}
...
function LegacyLobby({ code, seats, timer, setTimer, makeAi, setDifficulty, onBack, onStart, filled }: { code: string; seats: (LobbySeat & { color: string })[]; timer: number; setTimer: (value: number) => void; makeAi: (id: string) => void; setDifficulty: (id: string, difficulty: Difficulty) => void; onBack: () => void; onStart: () => void; filled: number }) {
  return <main className="app-shell lobby-shell"><header className="topbar"><button className="back-button" onClick={onBack}>← <span>Home</span></button><div className="brand"><span className="brand-mark">C</span><span>The Crow Game</span></div><div className="connection"><span className="status-dot" /> Private table</div></header><section className="lobby-header"><div><p className="eyebrow">Private table</p><h1>Gather your crows.</h1><p>Choose your seats, then start when everyone is ready.</p></div><div className="code-card"><span>JOIN CODE</span><strong>{code}</strong><button onClick={() => navigator.clipboard?.writeText(code)}>Copy code</button></div></section><section className="lobby-layout"><div className="panel seats-panel"><div className="panel-heading"><div><p className="eyebrow">The table</p><h2>{filled}/{seats.length} players ready</h2></div><span className="live-pill"><i /> Waiting</span></div><div className="seats-grid">{seats.map((seat) => <div className={`seat-card ${seat.status}`} key={seat.id}><div className="seat-top"><Avatar label={seat.name} color={seat.color} /><span className={`seat-badge team-${seat.team}`}>{seats.length === 5 ? `Seat ${seat.team}` : `Team ${seat.team}`}</span></div><strong>{seat.name}</strong>{seat.status === 'human' && seat.id === 'you' && <span className="seat-meta">That’s you · Host</span>}{seat.status === 'human' && seat.id !== 'you' && <span className="seat-meta">Connected</span>}{seat.status === 'open' && <><span className="seat-meta">Waiting for a player</span><button className="seat-action" onClick={() => makeAi(seat.id)}>Fill with AI +</button></>}{seat.status === 'ai' && <><span className="seat-meta">AI opponent</span><select value={seat.difficulty} onChange={(e) => setDifficulty(seat.id, e.target.value as Difficulty)}><option>Newbie</option><option>Average</option><option>Skilled</option></select></>}</div>)}</div><div className="team-note"><span>●</span><p>{seats.length === 5 ? 'Every player plays for themselves. The winning bidder calls a secret partner — the holder of the named card joins their side, and that card’s color becomes trump.' : 'Teams are assigned by the host. Team A and Team B will alternate partner positions around the table.'}</p></div></div><aside className="panel settings-panel"><div className="panel-heading"><div><p className="eyebrow">Table settings</p><h2>Rieman Rules</h2></div><span className="rules-icon">R</span></div><div className="setting"><span>Ruleset</span><strong>Rieman Rules{seats.length === 5 ? ' · 5 handed' : ''} <small>500 points</small></strong></div><div className="setting"><span>Turn timer</span><div className="stepper"><button onClick={() => setTimer(Math.max(10, timer - 5))}>−</button><strong>{timer}s</strong><button onClick={() => setTimer(Math.min(120, timer + 5))}>+</button></div></div><div className="rule-summary"><strong>Quick rules</strong><p>Crow is always trump · 110 points per hand · Dealer takes 65 if all pass.</p></div><button className="button primary full" disabled={seats.some((s) => s.status === 'open')} onClick={onStart}>Start the game <span>→</span></button><p className="small-help">Everyone can rejoin if they disconnect. The host can’t remove players once the game begins.</p></aside></section></main>
}
...
function Lobby({ code, name, onRename, seats, timer, setTimer, winningScore, setWinningScore, onSettingsChange, makeAi, setDifficulty, hostId, currentUserId, onBack, onStart, filled, onSwapSeats }: { code: string; name: string; onRename: (name: string) => void; seats: (LobbySeat & { color: string })[]; timer: number; setTimer: (value: number) => void; winningScore: number; setWinningScore: (value: number) => void; onSettingsChange: (value: number) => void; makeAi: (id: string) => void; setDifficulty: (id: string, difficulty: Difficulty) => void; hostId: string | null; currentUserId?: string; onBack: () => void; onStart: () => void; filled: number; onSwapSeats: (firstId: string, secondId: string) => void }) {
  ...
  const fiveHanded = seats.length === 5
  ...
  return <main className="app-shell lobby-shell">
    ...
    <section className="lobby-layout"><div className="panel seats-panel"><div className="panel-heading"><div><p className="eyebrow">The table</p><h2>{filled}/{seats.length} players ready</h2><p className="host-line">Leader: <strong>{host?.name ?? 'Table host'}</strong>{isHost ? ' · You' : ''}</p></div><span className="live-pill"><i /> Waiting</span></div><div className="seats-grid">{seats.map((seat) => <div className={`seat-card ${seat.status} ${moveFrom === seat.id ? 'swapping' : ''}`} key={seat.id}><div className="seat-top"><Avatar label={seat.name} color={seat.color} /><span className={`seat-badge team-${seat.team}`}>{fiveHanded ? `Seat ${seat.team}` : `Team ${seat.team}`}</span></div><strong>{seat.name}</strong>{seat.id === hostId && <span className="host-badge">Table leader</span>}{seat.status === 'human' && seat.id !== hostId && <span className="seat-meta">Connected</span>}{seat.status === 'open' && <><span className="seat-meta">Waiting for a player</span>{isHost && <button className="seat-action" onClick={() => makeAi(seat.id)}>Fill with AI +</button>}</>}{seat.status === 'ai' && <><span className="seat-meta">AI opponent</span>{isHost ? <select value={seat.difficulty} onChange={(e) => setDifficulty(seat.id, e.target.value as Difficulty)}><option>Newbie</option><option>Average</option><option>Skilled</option></select> : <span className="seat-meta">Set by the leader</span>}</>}{isHost && <button className="seat-move" onClick={() => handleSwapClick(seat.id)}>{moveFrom === seat.id ? 'Cancel' : moveFrom ? `Swap with ${seats.find((candidate) => candidate.id === moveFrom)?.name ?? 'player'}` : 'Move'}</button>}</div>)}</div><div className="team-note"><span>●</span><p>{fiveHanded ? 'Every player plays for themselves. The winning bidder calls a secret partner — the holder of the named card joins their side, and that card’s color becomes trump.' : (isHost ? (moveFrom ? 'Now choose a seat to swap with, or press Cancel on the picked-up seat.' : 'You are the table leader. Move swaps two players, and each takes their new seat’s team.') : `The table leader is ${host?.name ?? 'the host'}. They arrange seats, teams, and AI before starting.`)}</p></div></div><aside className="panel settings-panel"><div className="panel-heading"><div><p className="eyebrow">Table settings</p><h2>Rieman Rules</h2></div><span className="rules-icon">R</span></div><div className="setting ruleset-setting"><span>Ruleset</span><div className="ruleset-value"><strong>Rieman Rules {fiveHanded ? '· 5 handed' : ''} <small>{winningScore} points</small></strong><button className="rules-info-button" onClick={() => setShowRules(true)} aria-label="How to play Rieman Rules">?</button></div></div><div className="setting"><span>Winning score</span><div className="stepper"><button disabled={!isHost} onClick={() => { const next = Math.max(250, winningScore - 50); setWinningScore(next); onSettingsChange(next) }}>−</button><strong>{winningScore}</strong><button disabled={!isHost} onClick={() => { const next = Math.min(1000, winningScore + 50); setWinningScore(next); onSettingsChange(next) }}>+</button></div></div><div className="setting"><span>Turn timer</span><div className="stepper"><button disabled={!isHost} onClick={() => setTimer(Math.max(10, timer - 5))}>−</button><strong>{timer}s</strong><button disabled={!isHost} onClick={() => setTimer(Math.min(120, timer + 5))}>+</button></div></div><div className="rule-summary"><strong>Quick rules</strong><p>{fiveHanded ? 'Crow is always trump · 110 points per hand · Bidder calls a secret partner.' : 'Crow is always trump · 110 points per hand · Dealer takes 65 if all pass.'}</p></div><button className="button primary full" disabled={!canStart} onClick={onStart}>{isHost ? 'Start the game' : 'Waiting for the leader'} <span>→</span></button><p className="small-help">Everyone can rejoin if they disconnect. Only the table leader can change seats or start the game.</p></aside></section>
    {showRules && <RulesModal name="Rieman Rules" fiveHanded={fiveHanded} onClose={() => setShowRules(false)} />}
  </main>
}

function RulesModal({ name, onClose, fiveHanded = false }: { name: string; onClose: () => void; fiveHanded?: boolean }) {
  return <div className="modal-backdrop" onClick={onClose}><div className="modal-card rules-modal-card" onClick={(event) => event.stopPropagation()}>
    ...
    <div className="rules-body">
      ...
      {fiveHanded && <>
      <h4>The table</h4>
      <p>Five players, each playing for themselves. The winning bidder secretly names a partner, so the hand becomes a 2-vs-3 battle.</p>
      <h4>The deal</h4>
      <p>Each player receives 11 cards. The last 2 go face-down to a kitty in the middle of the table.</p>
      </>}
      <h4>Bidding</h4>
      <p>{fiveHanded ? 'Bids run from 60 to 105 in steps of 5 — raise the bid or pass. The highest bidder wins the kitty; if everyone passes, the dealer is forced to take the bid at 60.' : 'The player left of the dealer starts the bidding. Bids run from 65 to 110 in steps of 5 — raise the bid or pass. The highest bidder wins the kitty; if everyone passes, the dealer is forced to take the bid at 65.'}</p>
      <h4>Kitty &amp; trump</h4>
      {fiveHanded
        ? <p>The winning bidder picks up the kitty, then discards any two cards back down. Point cards — the Crow, 5s, 10s, and 14s — may never be discarded. Then they call a card they don’t hold; whoever holds it is their secret partner and that card’s color becomes trump. Calling the Crow means they name their own trump — whoever plays the Crow joins their side as partner.</p>
      ...
      <p>{fiveHanded ? 'The player left of the dealer leads any card, and everyone must follow that color if they can. The highest card of the led color wins the trick; trump beats plain colors, and the Crow beats everything. The winner of each trick leads the next until all 11 tricks are done. The partner stays hidden until they play the named card.' : 'The player left of the dealer leads any card, and everyone must follow that color if they can. The highest card of the led color wins the trick; trump beats plain colors, and the Crow beats everything. The winner of each trick leads the next until all 13 tricks are done.'}</p>
      <h4>Scoring</h4>
      <p>Captured cards earn points: the Crow is worth 10, 5s are worth 5, and 10s and 14s are worth 10 — 110 points in every hand. The bidding team must take at least what they bid, or that amount is subtracted from their score; the other side keeps every point they capture. {fiveHanded ? 'The trio splits its points evenly among its three players; the pair keeps its points as one team.' : ''} First to 500 wins the game.</p>
    </div>
    <div className="modal-actions"><button className="button primary" onClick={onClose}>Got it</button></div>
  </div></div>
}

function StatsModal({ stats, partnerNames, onClose }: { stats: PlayerStatistics | null; partnerNames: Record<string, string>; onClose: () => void }) {
  const colorLabel: Record<string, string> = { black: 'Black', red: 'Red', yellow: 'Yellow', green: 'Green' }
  const renderGrid = (source: { games_won: number; games_lost: number; games_unfinished: number; games_completed: number; hands_played: number; hands_bid: number; winning_bids: number; favorite_colors: Record<string, number>; favorite_partners?: Record<string, number> }, showPartner: boolean) => {
    ...
    const partners = showPartner
      ? Object.entries(source.favorite_partners ?? {})
          .filter(([id]) => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(id))
          .map(([id, count]) => ({ id, name: partnerNames[id] ?? 'Player', count }))
          .sort((a, b) => b.count - a.count || a.name.localeCompare(b.name))
      : []
    const maxPartner = partners.length ? Math.max(...partners.map(({ count }) => count)) : 0
    ...
    {partners.length > 0 && <div className="favorites"><p className="eyebrow">Favorite partner</p>{partners.map(({ id, name, count }) => <div className="favorite-row favorite-row-partner" key={id}><span title={name}>{name}</span><div className="favorite-bar"><i className="favorite-fill-partner" style={{ width: `${maxPartner ? (count / maxPartner) * 100 : 0}%` }} /></div><strong>{count}</strong></div>)}</div>}
    </>
  }
  const renderVariant = (label: string, players: Parameters<typeof renderGrid>[0], ai: Parameters<typeof renderGrid>[0]) => (
    <div className="stats-section">
      <p className="stats-variant-label">{label}</p>
      <div className="stats-section">
        <p className="stats-section-label">Versus players</p>
        {renderGrid(players, true)}
      </div>
      <div className="stats-section">
  ...
  return <div className="modal-backdrop" onClick={onClose}><div className="modal-card rules-modal-card stats-modal-card" onClick={(event) => event.stopPropagation()}>
    ...
    {stats ? <>
      {renderVariant('Rieman Rook · 4 handed', stats, { games_won: stats.ai_games_won, games_lost: stats.ai_games_lost, games_unfinished: stats.ai_games_unfinished, games_completed: stats.ai_games_completed, hands_played: stats.ai_hands_played, hands_bid: stats.ai_hands_bid, winning_bids: stats.ai_winning_bids, favorite_colors: stats.ai_favorite_colors, favorite_partners: stats.ai_favorite_partners })}
      {renderVariant('Rieman Rook · 5 handed', { games_won: stats.games5_won, games_lost: stats.games5_lost, games_unfinished: stats.games5_unfinished, games_completed: stats.games5_completed, hands_played: stats.hands5_played, hands_bid: stats.hands5_bid, winning_bids: stats.winning5_bids, favorite_colors: stats.favorite5_colors, favorite_partners: stats.favorite5_partners }, { games_won: stats.ai_games5_won, games_lost: stats.ai_games5_lost, games_unfinished: stats.ai_games5_unfinished, games_completed: stats.ai_games5_completed, hands_played: stats.ai_hands5_played, hands_bid: stats.ai_hands5_bid, winning_bids: stats.ai_winning5_bids, favorite_colors: stats.ai_favorite5_colors, favorite_partners: stats.ai_favorite5_partners })}
...
function sumLeaderboardNumbers(...sets: LeaderboardNumbers[]): LeaderboardNumbers {
  return sets.reduce((acc, set) => ({
    gamesWon: acc.gamesWon + set.gamesWon,
    gamesCompleted: acc.gamesCompleted + set.gamesCompleted,
    handsPlayed: acc.handsPlayed + set.handsPlayed,
    handsBid: acc.handsBid + set.handsBid,
    winningBids: acc.winningBids + set.winningBids,
  }), { gamesWon: 0, gamesCompleted: 0, handsPlayed: 0, handsBid: 0, winningBids: 0 })
}

function scopeNumbers(entry: LeaderboardEntry, scope: LeaderboardScope) {
  if (scope === 'players') return sumLeaderboardNumbers(entry.stats, entry.five)
  if (scope === 'ai') return sumLeaderboardNumbers(entry.ai, entry.aiFive)
  return sumLeaderboardNumbers(entry.stats, entry.five, entry.ai, entry.aiFive)
}
...
function SettingsScreen({ currentLogo, catalog, tokens, purchasedLogos, currentAnimation, purchasedAnimations, currentPlacement, purchasedPlacements, currentFont, purchasedFonts, onBack, onSelect, onPurchase, onSelectAnimation, onPurchaseAnimation, onSelectPlacement, onPurchasePlacement, onSelectFont, onPurchaseFont, displayName, onChangeDisplayName }: { currentLogo: string | null; catalog: CrowLogoRecord[]; tokens: number; purchasedLogos: string[]; currentAnimation: string | null; purchasedAnimations: string[]; currentPlacement: string | null; purchasedPlacements: string[]; currentFont: string | null; purchasedFonts: string[]; onBack: () => void; onSelect: (id: string) => void; onPurchase: (id: string) => void; onSelectAnimation: (id: string | null) => void; onPurchaseAnimation: (id: string) => void; onSelectPlacement: (id: string | null) => void; onPurchasePlacement: (id: string) => void; onSelectFont: (id: string | null) => void; onPurchaseFont: (id: string) => void; displayName: string; onChangeDisplayName: (name: string) => void }) {
  ...
  return <main className="app-shell settings-shell">
    ...
    <section className="panel settings-panel-lg">
      ...
      <div className="shop-section">
        <div className="shop-section-heading"><p className="eyebrow">Card fonts</p><h3>Set the tone.</h3><p className="settings-sub">A typeface applies to every numbered card you play, in your hand and on the table. Liquid and Aurora are premium at 20 tokens; the classic typefaces cost {COINS_PER_CARD_FONT}.</p></div>
        <div className="anim-grid">
...
function GameScreen({ game, sessionId, currentUserId, isHost, crowLogos, catalog, cardAnimation, placements, cardFonts, onRematch, onCloseLobby, onBid, onTrump, onDiscard, onCard, onCall, onBack, localMode = false }: { game: SessionState; sessionId: string | null; currentUserId?: string; isHost: boolean; crowLogos: Record<string, string | null>; catalog: CrowLogoRecord[]; cardAnimation: string | null; placements: Record<string, string | null>; cardFonts: Record<string, string | null>; onRematch: () => void; onCloseLobby: () => void; onBid: (amount: number | null) => void; onTrump: (color: CardColor) => void; onDiscard: (cardIds: string[]) => void; onCard: (cardId: string) => void; onCall: (cardId: string) => void; onBack: () => void; localMode?: boolean }) {
  ...
  const partner = game.players.find((player) => player.id === game.hand?.partnerId)
  const isMyTurn = currentPlayer?.id === currentUserId
  const isBidder = bidder?.id === currentUserId
  const dealer = game.players[game.hand?.dealerIndex ?? 0]
  const fiveHanded = isFiveHanded(game)
  const pairRevealed = fiveHanded && !game.hand?.alone && Boolean(game.hand?.partnershipRevealed && game.hand?.partnerId)
  const playerCount = game.players.length
  const kittySize = fiveHanded ? 2 : 5
  const scoreTeams: Team[] = fiveHanded ? ['A', 'B', 'C', 'D', 'E'] : ['A', 'B']
  const scoreLabelFor = (team: Team) => {
    if (!fiveHanded) return teamLabel(team, game, currentUserId)
    return game.players.find((candidate) => candidate.team === team)?.name ?? `Team ${team}`
  }
  const bidOptions = Array.from({ length: 10 }, (_, index) => (fiveHanded ? 60 : 65) + index * 5).filter((bid) => bid > (game.hand?.currentBid ?? 0))
  ...
  const activeTrick = game.hand?.tricks[game.hand.tricks.length - 1]?.cards.length === playerCount ? undefined : game.hand?.tricks[game.hand.tricks.length - 1]
  const lastTrick = [...(game.hand?.tricks ?? [])].reverse().find((trick) => trick.cards.length === playerCount)
  ...
  const capturedLive = (() => {
    if (!fiveHanded || game.hand?.alone === undefined) return null
    const pairSet = new Set(game.hand.alone ? [game.hand.bidderId!] : [game.hand.bidderId!, game.hand.partnerId!])
    let pair = 0
    let trio = 0
    for (const trick of game.hand?.tricks ?? []) {
      if (!trick.winnerId) continue
      const points = pointsInCards(trick.cards.map(({ card }) => card))
      if (pairSet.has(trick.winnerId)) pair += points; else trio += points
    }
    return { pair, trio }
  })()
  const tableStatus = (() => {
    ...
    if (!capturedLive) return null
    if (capturedLive.pair >= bid) return 'made'
    if (110 - capturedLive.trio < bid) return 'set'
    return null
  })()
  ...
  const toggleDiscard = (card: Card) => { if (card.kind === 'crow' || (card.kind === 'number' && [5, 10, 14].includes(card.value))) return; setSelectedDiscard((selected) => selected.includes(card.id) ? selected.filter((id) => id !== card.id) : selected.length < kittySize ? [...selected, card.id] : selected) }
  const gameOver = game.hand?.phase === 'complete' && game.status === 'completed'
  const phaseTitle = gameOver ? 'The table is done.' : game.hand?.phase === 'bidding' ? 'Make your bid.' : game.hand?.phase === 'calling' ? 'Name your partner.' : game.hand?.phase === 'trump' ? 'Choose trump.' : game.hand?.phase === 'kitty' ? 'Discard the kitty.' : 'The hand is live.'
  return <main className="app-shell game-shell">
    ...
    <section className="game-header"><div><p className="eyebrow">Rieman Rules{fiveHanded ? ' · 5 handed' : ''} · Hand {game.handNumber + 1}</p><h1>{phaseTitle}</h1></div><div className={`scoreboard ${fiveHanded ? 'scoreboard-five' : ''}`}>{scoreTeams.map((team) => <div key={team}><span>{scoreLabelFor(team)}</span><strong>{game.scores[team] ?? 0}</strong></div>)}</div></section>
    {gameOver && <GameResult game={game} currentUserId={currentUserId} isHost={isHost} onRematch={onRematch} onCloseLobby={onCloseLobby} />}
    {!gameOver && game.hand?.phase === 'complete' && <div className={`result-banner ${game.hand.bidMade ? '' : 'failed-bid'}`}>{fiveHanded ? <><strong>{game.hand.alone ? (game.hand.bidMade ? 'The lone crow made the bid' : 'The lone crow failed the bid') : (game.hand.bidMade ? 'The pair made the bid' : 'The pair failed the bid')}</strong><span>Bid {game.hand.currentBid} · Pair {game.hand.pairPoints ?? 0} · Trio {game.hand.trioPoints ?? 0}</span></> : <><strong>{game.hand.bidMade ? `${teamLabel(game.hand.bidderTeam ?? 'A', game, currentUserId)} made the bid` : `${teamLabel(game.hand.bidderTeam ?? 'A', game, currentUserId)} failed the bid`}</strong><span>Bid {game.hand.currentBid} · Captured {game.hand.teamPoints?.[game.hand.bidderTeam ?? 'A'] ?? 0} · Score {game.hand.scoreDelta?.[game.hand.bidderTeam ?? 'A'] ?? 0}</span></>}</div>}
    {!gameOver && <section className={`game-board ${fiveHanded ? 'five-handed' : ''} ${tableStatus ? `table-${tableStatus}` : ''}`}><div className={`table-center ${game.hand?.trumpColor ? `table-trump-${game.hand.trumpColor}` : ''}`}><strong>{game.hand?.phase === 'bidding' ? game.hand.currentBid ?? 'No bid' : fiveHanded ? `${game.hand?.currentBid ?? '—'} pts` : teamLabel(bidder?.team ?? 'A', game, currentUserId)}</strong>{game.hand?.phase !== 'bidding' && <small>{fiveHanded ? 'pair vs trio' : `${game.hand?.currentBid ?? '—'} points`}</small>}{game.hand?.bidderId && <div className="table-hand-points">{fiveHanded ? (game.hand.alone === undefined || !game.hand.partnershipRevealed ? <strong>{game.hand.alone ? 'Flying solo' : 'Partner hidden'}</strong> : <><strong>Pair {capturedLive?.pair ?? 0}</strong><strong>Trio {capturedLive?.trio ?? 0}</strong></>) : <><strong>{teamLabel('A', game, currentUserId)} {capturedPointsForTeam(game, 'A')}</strong><strong>{teamLabel('B', game, currentUserId)} {capturedPointsForTeam(game, 'B')}</strong></>}</div>}{fiveHanded && game.hand?.calledCard && <div className="called-line"><span>Called</span><span className={`called-card ${game.hand.calledCard.kind === 'crow' ? 'crow-card' : `color-${game.hand.calledCard.color}`}`}><strong>{game.hand.calledCard.kind === 'crow' ? 'C' : game.hand.calledCard.value}</strong><small>{game.hand.calledCard.kind === 'crow' ? 'Crow' : game.hand.calledCard.color}</small></span></div>}</div>{showShuffle && <ShuffleAnimation />}<TableCards trick={tableTrick} players={game.players} visualPosition={visualPosition} biddingStatus={game.hand?.phase === 'bidding' ? `${currentPlayer?.name ?? 'Player'} is bidding` : undefined} crowLogos={crowLogos} catalog={catalog} placements={placements} cardFonts={cardFonts} />{game.players.map((player, index) => { const bid = latestBids.get(player.id); const position = visualPosition(index); const inPair = pairRevealed && (player.id === bidder?.id || player.id === partner?.id); const inTrio = pairRevealed && !inPair; const flyingSolo = fiveHanded && Boolean(game.hand?.alone) && player.id === bidder?.id; return <div className={`player-position player-${position} ${player.id === currentPlayer?.id ? 'is-turn' : ''} ${inPair ? 'pair-member' : inTrio ? 'trio-member' : ''} ${flyingSolo ? 'solo-member' : ''}`} key={player.id}><Avatar label={player.name} color={avatarColors[index]} />{player.id === dealer?.id && <span className="dealer-chip">Dealer</span>}{inPair && <span className="pair-chip">Pair</span>}{inTrio && <span className="trio-chip">Trio</span>}{flyingSolo && <span className="solo-chip">Solo</span>}<span>{player.name}{player.id === currentUserId ? ' · You' : ''}</span><small>{player.isAi ? 'AI' : ''}</small>{game.hand?.phase === 'bidding' && <span className={`table-bid-status ${player.id === currentPlayer?.id ? 'bidding-now' : ''}`}>{player.id === currentPlayer?.id && 'Bidding'}{player.id !== currentPlayer?.id && bid && (bid.passed ? 'Passed' : `Bid ${bid.amount}`)}{player.id !== currentPlayer?.id && !bid && 'Not bid'}</span>}</div> })}</section>}
    {!gameOver && game.hand?.phase === 'calling' && <section className="action-panel"><p className="eyebrow">Name your partner</p><h2>{isBidder ? 'Pick a card you don’t hold.' : `${bidder?.name ?? 'The winning bidder'} is naming a partner`}</h2>{isBidder ? <CallPanel hand={bidder?.hand ?? []} onCall={onCall} /> : <p className="muted-note">Whoever holds the card you call joins your side — its color becomes trump. Call the Crow to name your own trump — whoever plays it becomes your partner.</p>}</section>}
    ...
    {!gameOver && game.hand?.phase === 'complete' && <section className="next-hand-panel"><div><p className="eyebrow">Hand complete</p><h2>Scores: {scoreTeams.map((team) => `${scoreLabelFor(team)} ${game.scores[team] ?? 0}`).join(' · ')}</h2></div><p className="muted-note">Dealing the next hand…</p></section>}
...
function CallPanel({ hand, onCall }: { hand: Card[]; onCall: (cardId: string) => void }) {
  const held = new Set(hand.map((card) => card.id))
  const colors = ['black', 'red', 'yellow', 'green'] as CardColor[]
  const numbers = Array.from({ length: 14 }, (_, index) => index + 1)
  return <div className="call-options">{colors.map((color) => numbers.map((value) => { const id = `${color}-${value}`; const disabled = held.has(id); return <button className={`call-card color-${color}`} disabled={disabled} key={id} onClick={() => onCall(id)}><strong>{value}</strong><small>{color}</small></button> }))}<button className="call-card crow-card" disabled={held.has('crow')} onClick={() => onCall('crow')}><strong>C</strong><small>Crow</small></button></div>
}
...
function GameResult({ game, currentUserId, isHost, onRematch, onCloseLobby }: { game: SessionState; currentUserId?: string; isHost: boolean; onRematch: () => void; onCloseLobby: () => void }) {
  const fiveHanded = isFiveHanded(game)
  const myTeam = game.players.find((player) => player.id === currentUserId)?.team
  const winnerIds = fiveHanded ? (game.hand?.winnerPlayerIds ?? []).slice(0, 1) : (game.hand?.winnerPlayerIds ?? [])
  const won = fiveHanded ? winnerIds.includes(currentUserId ?? '') : myTeam === game.hand?.gameWinner
  const title = (() => {
    if (!fiveHanded) return `${teamLabel(game.hand?.gameWinner ?? 'A', game, currentUserId)} wins`
    const winnerNames = winnerIds.map((id) => game.players.find((player) => player.id === id)?.name ?? 'Player')
    if (winnerNames.length === 0) return 'The table is done.'
    if (winnerNames.length === 1) return `${winnerNames[0]} wins`
    if (winnerNames.length === 2) return `${winnerNames[0]} & ${winnerNames[1]} win`
    return `${winnerNames.slice(0, -1).join(', ')} & ${winnerNames[winnerNames.length - 1]} win`
  })()
  const confetti = createConfetti()
  return <section className={`game-result ${won ? 'game-won' : 'game-lost'}`}>{won && <div className="confetti" aria-hidden>{confetti.map((piece) => <i key={piece.id} style={{ left: piece.left, animationDelay: piece.delay, background: piece.color, transform: `rotate(${piece.rotation})` }} />)}</div>}<div className="result-icon">{won ? '★' : '○'}</div><p className="eyebrow">Game complete</p><h2>{title}</h2><p>{won ? `You took the table and earned +${tokensForWinningScore(game.winningScore)} tokens.` : 'The cards had other plans this time.'}</p>{isHost ? <div className="result-actions"><button className="button primary" onClick={onRematch}>Rematch →</button><button className="secondary-button" onClick={onCloseLobby}>Close lobby</button></div> : <p className="muted-note">Waiting for the table leader to choose a rematch or close the lobby.</p>}</section>
}

function TableCards({ trick, players, visualPosition, completed, biddingStatus, crowLogos, catalog, placements, cardFonts }: { trick?: { cards: Array<{ playerId: string; card: Card }>; winnerId?: string }; players: SessionState['players']; visualPosition: (index: number) => number; completed?: boolean; biddingStatus?: string; crowLogos: Record<string, string | null>; catalog: CrowLogoRecord[]; placements: Record<string, string | null>; cardFonts: Record<string, string | null> }) {
  if (!trick?.cards.length) return <div className="table-empty">{biddingStatus ?? 'Cards played this trick will appear here.'}</div>
  const trickComplete = completed ?? Boolean(players.length > 0 && trick.cards.length === players.length)
...
function BidHistory({ game, currentPlayerId }: { game: SessionState; currentPlayerId?: string }) {
  ...
  return <section className="bid-history"><div className="history-heading"><div><p className="eyebrow">Bidding</p><h2>Current bid: {game.hand?.currentBid ?? '—'}</h2></div><span>{passedPlayers.size} passed · {game.players.length - passedPlayers.size} eligible</span></div><div className={`bid-players ${game.players.length === 5 ? 'bid-players-five' : ''}`}>{game.players.map((player, index) => { const latest = [...bids].reverse().find((bid) => bid.playerId === player.id); return <div className={`bid-player ${player.id === currentPlayerId ? 'active' : ''}`} key={player.id}><Avatar label={player.name} color={avatarColors[index]} /><div><strong>{player.name}{player.id === currentPlayerId ? ' · Up now' : ''}</strong><small>{latest ? latest.passed ? 'Passed — out this hand' : `Bid ${latest.amount}` : 'Not bid yet'}</small></div></div> })}</div></section>
}
